...
(async()=>{
  ...
  r=JSON.parse(await ev(ws,`JSON.stringify({sent:window.__calls.filter(c=>/room\\/session/.test(c.url)).map(c=>c.body.action), row:(document.querySelector('.sesslist li')||{}).textContent, mins:sessMinsToday(), card:document.getElementById('view-now').innerText, today:[...document.querySelectorAll('#view-now .item .t')].map(x=>x.textContent).find(x=>/^Study/.test(x)), count:(document.querySelector('#view-now h2.sec .count')||{}).textContent, line:(document.querySelector('.roomline .txt')||{}).textContent})`));
  const sameDay = new Date(Date.now()+90*60e3).toDateString()===new Date().toDateString();   // near midnight the session is tomorrow's: then it is on the Classmates card, not in today's budget
  check('S2 I\'m in: the session is on Today with who and 90 minutes in the budget, and the course line names it', r.sent.join()==='in' && /3 going/.test(r.row) && /Can't make it/.test(r.row) && (sameDay ? (r.mins===90 && /^Study ECN 212 with Maya R. and Jordan P./.test(r.today||'') && /^2 things/.test(r.count)) : (r.mins===0 && /ECN 212 study session/.test(r.card||'') && /you're in/i.test(r.card||''))) && /Session .*you're in/.test(r.line), {sent:r.sent, mins:r.mins, today:r.today, count:r.count, line:r.line.slice(0,80)});
